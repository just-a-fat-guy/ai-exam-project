...
def _pdf_dependencies_available() -> tuple[bool, str]:
    """Check whether the native libraries required by WeasyPrint are present."""
    ...
    required_libraries = {
        "gobject-2.0": "glib",
        "pango-1.0": "pango",
        "pangocairo-1.0": "pango+cairo",
        "cairo": "cairo",
    }
    missing = [
        package_name
        for library_name, package_name in required_libraries.items()
        if find_library(library_name) is None
    ]
    if missing:
        missing_list = ", ".join(sorted(set(missing)))
        return False, f"missing native PDF dependencies: {missing_list}"
...
async def write_md_to_pdf(text: str, filename: str = "") -> str:
    ...
    dependencies_ok, reason = _pdf_dependencies_available()
    if not dependencies_ok:
        print(f"Skipping PDF export: {reason}")
        return ""
