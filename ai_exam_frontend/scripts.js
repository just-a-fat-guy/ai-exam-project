const GPTResearcher = (() => {
  ...
  const checkCookiesEnabled = () => {
    try {
      ...
      if (!cookieEnabled) {
        ...
        showToast("浏览器已禁用 Cookie，研究历史将改用 localStorage 保存。", 5000);
      } else {
  ...
  const renderHistoryEntries = () => {
    ...
    if (!conversationHistory || conversationHistory.length === 0) {
      historyEntries.innerHTML = '<p class="text-center mt-4 text-muted">当前还没有研究历史。</p>';
      return;
    }
  ...
  const loadResearchEntry = (index) => {
    ...
    // Inform user
    showToast('已载入这条研究的参数，你可以重新开始研究。');
  }
  ...
  function setCookie(name, value, days) {
    ...
    if (cookieSize > MAX_COOKIE_SIZE) {
      ...
      showToast('警告：历史记录过大，超出 Cookie 限制，最旧的记录将被移除。');
  ...
  const checkCookieStatus = () => {
    ...
    if (conversationCookie) {
      ...
      try {
        ...
        showToast(`已发现 Cookie：${kilobyteSize}KB，${entryCount} 条研究记录`);
        console.debug(`Cookie size: ${byteSize} bytes, ${kilobyteSize}KB`);
        console.debug(`Cookie entries: ${entryCount}`);
      } catch (e) {
        showToast(`已发现 Cookie，但数据无效：${kilobyteSize}KB`);
  ...
  const handleFileImport = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (conversationHistory && conversationHistory.length > 0) {
          if (confirm(`当前已有 ${conversationHistory.length} 条研究记录。你想如何处理？
- 点击“确定”：将导入记录与现有历史合并
- 点击“取消”：用导入记录替换现有历史`)) {
  ...
  const initChat = () => {
    ...
    // Add welcome message
    addChatMessage('我可以继续回答与你这份研究报告相关的问题。你想先问什么？', false);
  }
  ...
  const initSpeechRecognition = (button, inputElement) => {
    ...
    recognition.onerror = (event) => {
      ...
      if (event.error === 'not-allowed') {
        ...
        showToast('语音识别出错：' + event.error, 3000);
      }
    };
  ...
  const reconnectWebSocket = (message = null) => {
    ...
    if (reconnectAttempts >= maxReconnectAttempts) {
      ...
      addChatMessage(`重连尝试 ${maxReconnectAttempts} 次后仍失败，请刷新页面。`, false);
      return false;
    }
  ...
  const validateMCPConfig = () => {
    ...
    try {
      ...
      parsed.forEach((server, index) => {
        if (!server.name) {
          errors.push(`第 ${index + 1} 个 Server：缺少 name`);
        }
        if (!server.command && !server.connection_url) {
          errors.push(`第 ${index + 1} 个 Server：缺少 command 或 connection_url`);
        }
      });
      ...
      mcpConfigStatus.textContent = `JSON 有效 ✓（共 ${parsed.length} 个 Server）`;
  ...
  const createMCPInfoModal = () => {
    ...
    modal.innerHTML = `
      <div class="mcp-info-content">
        ...
        <p>MCP 允许 GPT Researcher 通过标准化协议连接外部工具与数据源。</p>
        
        <h4 class="highlight">优势：</h4>
        <ul>
          <li><span class="highlight">访问本地数据：</span>连接数据库、文件系统与 API</li>
          <li><span class="highlight">使用外部工具：</span>集成 Web 服务和第三方工具</li>
          <li><span class="highlight">扩展能力：</span>通过 MCP Server 增加自定义功能</li>
          <li><span class="highlight">保持安全：</span>通过认证与授权控制访问范围</li>
        </ul>

        <h4 class="highlight">快速开始：</h4>
        <ul>
          <li>先勾选上方的 MCP 开关</li>
          <li>点击预设按钮，把预配置的 Server 加入 JSON</li>
          <li>或者直接粘贴你自己的 MCP JSON 配置</li>
          <li>开始研究后，MCP 会按默认最优方式运行</li>
        </ul>

        <h4 class="highlight">配置格式：</h4>
        <p>每个 MCP Server 都应该是一个 JSON 对象，通常包含以下字段：</p>
        <ul>
          ...
          <li><span class="highlight">args：</span>命令参数数组</li>
  ...
  const addMCPPreset = (preset) => {
    ...
    try {
      ...
      if (existingIndex !== -1) {
        ...
        showToast(`已更新 ${preset} 的 MCP Server 配置。`);
      } else {
        // Add new server
        currentConfig.push(config);
        showToast(`已添加 ${preset} 的 MCP Server 配置。`);
      }
