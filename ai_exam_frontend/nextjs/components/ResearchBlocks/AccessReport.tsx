...
const AccessReport: React.FC<AccessReportProps> = ({ accessData, chatBoxSettings, report, onShareClick }) => {
  ...
  return (
    <div className="container apple-panel my-5 rounded-[30px] border-white/8 p-6">
      <div className="flex flex-col items-center">
        <h3 className="mb-2 text-lg font-semibold text-white/88">获取你的研究报告</h3>
        <p className="mb-5 text-sm text-white/44">下载、分享或导出本次研究结果。</p>
        
        <div className="flex flex-wrap justify-center gap-3">
          {accessData.pdf && (
            <a 
              href={getReportLink('pdf')} 
              className="apple-button-secondary flex items-center gap-2 rounded-full px-5 py-3 text-sm font-medium uppercase"
              target="_blank"
          ...
          {accessData.docx && (
            <a 
              ...
              className="apple-button-primary flex items-center gap-2 rounded-full px-5 py-3 text-sm font-medium uppercase"
              target="_blank"
              rel="noopener noreferrer">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
