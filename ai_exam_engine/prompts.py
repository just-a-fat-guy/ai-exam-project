...
这个文件不直接“调用模型”，而是负责把生成能力里要用到的提示词模板集中定义好。

可以把它理解成写作链路中的“文案工厂”：
1. 上层调用方先根据当前任务判断“要写哪一类内容”
2. 再从这里拿到对应的 prompt
3. 最终交给 `utils/llm.py` 去真正请求模型

因此，这一层的核心职责不是业务编排，而是：
- 统一 prompt 风格与结构
- 根据报告类型返回不同模板
- 根据模型家族适配不同的上下文格式
"""
...
class PromptFamily:
    ...
    这个类是整个项目 prompt 体系的中心抽象。它把“如何给模型下指令”
    统一封装成一组方法，供研究阶段和写作阶段反复调用。

    可以把这里的方法分成两类：

    1. Prompt Generators
       这一类方法通常和报告类型直接相关，签名也比较统一。
       例如 research report、outline report、deep research report。
       它们通常通过 `get_prompt_by_report_type()` 间接获取。
    ...
    2. Prompt Methods
       ...
       - 写引言 / 写结论
       - 生成子主题标题
       这类方法会被 agent 或 action 代码直接调用。

    为什么叫 family：
    - 因为同一套业务动作，在不同模型家族上可能需要不同 prompt 风格
    - 例如 Granite 模型会有自己更偏好的 documents 格式
    - 因此项目用“家族”而不是“单个 prompt 函数”来组织模板
    """

    def __init__(self, config: Config):
        """保存配置对象，供派生类在生成 prompt 时做适配。

        参数说明：
            config:
                当前研究任务的配置对象。不同 prompt family 可以根据
                这里的模型名、provider、语言等信息，决定采用哪种模板细节。
        """
        self.cfg = config
...
- Suitable for both digital viewing and printing"""
    ...
    def generate_search_queries_prompt(
        ...
        参数说明：
            question:
                当前要展开研究的问题。普通报告里它通常就是用户原始 query；
                详细报告里它也可能是某个子问题。
            parent_query:
                主问题。仅在 detailed report / subtopic report 等需要把
                “主问题 + 子问题”一起考虑的场景下才有意义。
            report_type:
                当前报告类型。它会影响 prompt 里最终组装出来的任务描述。
            max_iterations:
                最多要让模型生成多少条搜索查询。
            context:
                初始搜索已经拿到的实时信息。这里把它塞回 prompt，
                是为了让模型生成更贴近当前语境的后续搜索词。
    ...
    def generate_report_prompt(
        ...
        参数说明：
            ...
            total_words:
                期望的最小字数。
            tone:
                语气对象，例如 Objective / Formal / Analytical。
            language:
                报告输出语言。
        ...
        这是最常用的写作模板。`report_generation.py` 在普通研究报告场景下，
        最终就会走到这里来拼装 user prompt。
        """
...
"""
    ...
    def generate_subtopics_prompt() -> str:
        """生成“从研究数据里提炼子主题”的 prompt 模板。

        注意：
            这里返回的是一个带占位符的模板字符串，而不是最终 prompt。
            真正调用时，会由 `utils/llm.py.construct_subtopics()` 用
            `PromptTemplate` 把 `{task}`、`{data}`、`{subtopics}`、
            `{max_subtopics}` 等变量填进去。
        """
        return """
    ...
    def generate_subtopic_report_prompt(
        ...
        参数说明：
            current_subtopic:
                当前正在写的子主题。
            existing_headers:
                已经写过的章节标题列表，用来避免结构重复。
            relevant_written_contents:
                已经写好的相关正文片段，用来避免内容重复。
            main_topic:
                整份大报告的总主题。
            context:
                当前子主题可用的研究上下文。
            report_format / max_subsections / total_words / tone / language:
                分别控制引用格式、子节数量上限、字数、语气和语言。

        返回值：
            str:
                子主题报告写作 prompt。

        这类 prompt 的重点不是“从零写一篇报告”，而是：
        - 在总报告框架下写一个局部章节
        - 避免与已写章节冲突
        - 保持与主主题强相关
        """
...
"""
    ...
    def generate_report_introduction(question: str, research_summary: str = "", language: str = "english", report_format: str = "apa") -> str:
        ...
        这里要求模型只写“引言部分”，而不是整篇报告。
        上层通常会在 detailed report 模式下单独调用它，为总报告补一个开头。
        """
...
class GranitePromptFamily(PromptFamily):
    """IBM Granite 模型族的 prompt 适配层。

    这个类本身更像一个“分发器”：
    - 根据当前配置里的模型版本
    - 决定把文档格式相关逻辑委托给哪个 Granite 子类
    """
...
class Granite3PromptFamily(PromptFamily):
    """Granite 3.0 - 3.2 系列的 prompt 适配。

    重点不是改写所有 prompt 内容，而是调整“文档上下文如何组织”的格式，
    让模型更容易正确读取多段文档。
    """
...
class Granite33PromptFamily(PromptFamily):
    """Granite 3.3 的 prompt 适配。

    这一版对 documents 结构有更明确的 template，因此这里单独实现。
    """
...
# 这里开始是 prompt 工厂层。
# 前面的类负责“定义模板”，这里负责“根据报告类型 / prompt family 名称
# 把正确的模板函数或 family 实例取出来”。
...
def get_prompt_by_report_type(
    ...
    """按报告类型选择对应的 prompt 生成函数。

    参数说明：
        report_type:
            当前报告类型，例如 `research_report`、`outline_report`、
            `subtopic_report`、`deep_research`。
        prompt_family:
            当前生效的 prompt family，可以是类也可以是实例。

    返回值：
        callable:
            与该报告类型对应的 prompt 生成函数。

    这一步的意义是把“报告类型”与“具体 prompt 函数”解耦。
    上层只需要知道自己在写哪种报告，不需要手动 if/else 去找函数名。
    """
...
def get_prompt_family(
    ...
    """根据名称获取 prompt family 实例。

    参数说明：
        prompt_family_name:
            prompt family 的枚举值或字符串名称。
        config:
            当前任务配置对象，构造 family 时会注入进去。

    返回值：
        PromptFamily:
            可直接使用的 prompt family 实例。

    这是 prompt 层的总入口。`agent.py` 初始化时会先选定一个 family，
    后续研究与写作流程都会复用它。
    """
